...
public class AlgoW_PFI_Apriori <E> {
    ...
    public Set<Set<E>> runAlgorithm(double scaleFactor, boolean isProbabilityModelBase){
        ...
        Set<Set<E>> wPFI_k = Scan_Find_Size_k_wPMFI(I); // size k = 1
        System.out.println("size-1 wPFI;");
        System.out.println(wPFI_k);

        // Add WPFIk-1 into WPFI.
        List<Set<Set<E>>> wPFI = new ArrayList<>();
        wPFI.add(wPFI_k);
        ...
        while(!wPFI_k.isEmpty()){
            ...
            wPFI_k = Scan_Find_Size_k_wPMFI(Ck); // size k
        ...
        //find wPMFI from wPFI
        Set<Set<E>> wPMFI = findAllWPMFI(wPFI);

        return wPMFI;
    }

    /**
     * This method find all wPMFI from wPFI
     * @param wPFI list of wPFI
     * @return itemset is wPMFI
     */
    private Set<Set<E>> findAllWPMFI(List<Set<Set<E>>> wPFI) {
        Set<Set<E>> wPMFI = new HashSet<>();

        //find itemset X is wPMFI;
        //itemset X is wPMFI: not find itemset Y so that Y is wPFI and Y covered itemset X
        for(int i = 0; i < wPFI.size() - 1; i++){
            Set<Set<E>> wPFI_k = wPFI.get(i);
            Set<Set<E>> wPFI_k_Plus_1 = wPFI.get(i + 1);

            //traversal 2 set size-k and size-(k+1)
            //if itemset in size-(k+1) didn't cover itemset size_k -> itemset size_k is wPMFI
            for(Set<E> itemsetWPFI_k : wPFI_k){
                boolean isContainInLargerItemset = false;
                for (Set<E> itemsetWPFI_k_Plus_1 : wPFI_k_Plus_1){
                    if(itemsetWPFI_k_Plus_1.containsAll(itemsetWPFI_k)){
                        isContainInLargerItemset = true;
                        break;
                    }
                }
                if(!isContainInLargerItemset){
                    wPMFI.add(itemsetWPFI_k);
                }
            }
        }

        //add last set in wPFI; because it alway maximal
        int lastIndexInWPFI = wPFI.size() - 1;
        wPMFI.addAll(wPFI.get(lastIndexInWPFI));
     ...
     */
    private Set<Set<E>> Scan_Find_Size_k_wPMFI(Set<Set<E>> candidateK) {
        ...
        for(Set<E> itemset : candidateK){
            ...
            if(probabilisticFrequentItemset
                ...
                //System.out.println(itemset +" --_ " + probabilisticFrequentItemset.calculateWeightedProbabilisticItemset(minSupport));
