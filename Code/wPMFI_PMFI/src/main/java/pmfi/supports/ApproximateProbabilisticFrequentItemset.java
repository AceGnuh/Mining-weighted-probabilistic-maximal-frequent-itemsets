...
//        this.inputItemset = inputItemset;
     ...
     */
    public int calculateWeightedProbabilisticSupport(double minProbabilisticConfidence) {
        ...
        double weightedItemset = weightedTable.getWeighedItemset(inputItemset);

        if(weightedItemset <= minProbabilisticConfidence){
            return 0;
        }
        ...
        double inverseCumulativeProbability = normalDistribution
                .inverseCumulativeProbability(1.0 - minProbabilisticConfidence / weightedItemset);
    ...
    public boolean isProbabilisticFrequentItemset(double minSupport, double minProbabilisticConfidence) {
        return this.calculateWeightedProbabilisticSupport(minProbabilisticConfidence) >= minSupport;
    }
        ...
        if(this.isProbabilisticFrequentItemset(minSupport, minProbabilisticConfidence)){
            ...
            for(E distinctItem : uncertainDatabase.getDistinctItem()){
                ...
                if(tempDistinctItem.containsAll(distinctIemInput) && !tempDistinctItem.equals(distinctIemInput)){
                    ApproximateProbabilisticFrequentItemset<E> frequentItemset = new ApproximateProbabilisticFrequentItemset<>(this.uncertainDatabase, weightedTable, new ArrayList<>(tempDistinctItem));
